...
# TODO: remove this import once we update the code in the aura repo to directly
# import this function from gsim.include.utils.files.
from .include.utils.files import save_to_results_text_file
...
_current_run: tuple[type,
                    str] | None = None  # (experiment set class, experiment id)
...
def results_folder() -> str:
    ...
    if _current_run is None:
        raise RuntimeError(
            "results_folder() called outside of run_experiment.")
    cls, _experiment_id = _current_run
    return cls.experiment_set_data_folder()


def results_file_path(file_key: str = "", file_type: str = "pk") -> str:
    """`<results_folder>/experiment_<id>[_<file_key>].<file_type>` for the running experiment."""
    if _current_run is None:
        raise RuntimeError(
            "results_file_path() called outside of run_experiment.")
    cls, experiment_id = _current_run
    f_name = cls._experiment_id_to_f_name(experiment_id)
    if file_key:
        f_name = f"{f_name}_{file_key}"
